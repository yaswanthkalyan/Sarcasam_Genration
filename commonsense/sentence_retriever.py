...
def getSentencesOnline(keyword):
    ...
    print(keyword, "online")
    originalurl = '"https://sentencedict.com/'+keyword+'.html"'
    url = originalurl
    command = 'wget '+originalurl+' -q -nv -O ./temp/sent.txt'
    os.system(command)
    f = open('./temp/sent.txt','r',encoding ='ISO-8859-1').read()
    c = 1
    if isPageInValid(f):
        print(keyword, "Invalidddd")
        return []
    else:
        s = []
        while True:
            if c>1:
                ...
                os.system('wget '+url+' -q -nv -O ./temp/sent'+str(c)+'.txt')
                f = open('./temp/sent'+str(c)+'.txt','r',encoding ='ISO-8859-1').read()
                if c>getLast(f):
                    break
            c = c+1
        if flag:
...
def getSentences(keyword,utterance):
    ...
    if len(keyword) <= 1:
        noContext = []
        noContext.append("NoContext")
        print(noContext)
        return noContext
    elif keyword in concepts:
        # print(keyword, "keyConcept")
        retrieval_corpus = open('./data/corpus.txt', 'r', encoding='utf-8')
